...
dfx deploy evm_rpc
dfx deploy --with-cycles 10_000_000_000_000 coprocessor_canister --argument='(
  record {
    ecdsa_key_id = record {
      name = "dfx_test_key";
...
export EVM_ADDRESS=$(dfx canister call coprocessor_canister get_evm_address | awk -F'"' '{print $2}')
